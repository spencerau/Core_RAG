...
class SummaryIndexer:
    ...
    def _summary_enabled_collections(self):
        """Return only the Qdrant collection names that have summary_enabled: true."""
        coll_cfg = self.config.get('collection_config', {})
        all_collections = self.config['qdrant']['collections']
        enabled = []
        for key, name in all_collections.items():
            cfg = coll_cfg.get(key, {})
            if cfg.get('summary_enabled', not coll_cfg):
                enabled.append(name)
        return enabled

    def _ensure_summary_collections(self):
        for collection_name in self._summary_enabled_collections():
    ...
    def generate_summary(self, text: str, title: str = None) -> str:
        ...
        try:
            ...
            resp = self.ollama_api.chat(
                model=self.llm_config.get('primary_model', 'llama3.2'),
                messages=messages,
                stream=False,
                think=False,
...
def ingest_summaries(data_directories: List[str] = None, collection_name: str = None) -> Dict:
    ...
    if data_directories is None:
        data_directories = [p for _, p in config.get('data', {}).items() if isinstance(p, str) and os.path.exists(p)]
